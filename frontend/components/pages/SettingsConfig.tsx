...
import { SensorsHealthTab } from '../settings/SensorsHealthTab';
...
export const SettingsConfig: React.FC<SettingsConfigProps> = ({ onNavigate }) => {
  ...
  const [activeTab, setActiveTab] = useState<'profile' | 'sensors' | 'safety' | 'diagnostics' | 'health'>('profile');
    ...
    <div className="p-4 lg:p-6 space-y-6 max-w-7xl mx-auto animate-fade-in-up">
      ...
      <div className="glass-panel p-1.5 rounded-2xl border border-white/10 flex flex-wrap items-center gap-1.5">
        ...
        <button
          ...
          className={`flex items-center space-x-2 px-4 py-2 rounded-xl text-xs font-mono font-medium transition-all ${
            activeTab === 'health'
              ? 'bg-[#10b981] text-[#04131d] font-bold shadow-lg shadow-[#10b981]/20'
              : 'text-gray-400 hover:text-white hover:bg-white/5'
          }`}
        >
          <HeartPulse className={`w-4 h-4 ${activeTab === 'health' ? 'text-[#04131d]' : 'text-[#10b981]'}`} />
      ...
      {/* Tab 5: Sensors Health (Plant 1 & Plant 2) */}
      {activeTab === 'health' && (
        <SensorsHealthTab />
      )}
    </div>
