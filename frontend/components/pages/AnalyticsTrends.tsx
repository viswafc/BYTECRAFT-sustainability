...
export const AnalyticsTrends: React.FC<AnalyticsTrendsProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="p-4 lg:p-6 space-y-6 max-w-7xl mx-auto animate-fade-in-up">
      ...
      <div className="glass-panel rounded-2xl p-5 border border-[#28D7FF]/30 space-y-4 shadow-xl shadow-black/40">
          ...
          <div className="flex items-center space-x-2.5">
            ...
            <div>
              ...
              <p className="text-[11px] font-mono text-gray-400">
                Continuous baseline comparison vs SCADA flow telemetry
              </p>
            </div>
          </div>

          <div className="flex items-center space-x-4 text-xs font-mono">
            {/* Luminous Cyan Expected Baseline Legend */}
            <div className="flex items-center space-x-2 px-2.5 py-1 rounded-lg bg-[#00E5FF]/10 border border-[#00E5FF]/30">
              <span className="w-3.5 h-1 rounded-full bg-[#00E5FF] shadow-[0_0_8px_#00E5FF]" />
              <span className="text-[#00E5FF] font-bold">Expected Baseline</span>
            </div>

            {/* Vibrant Coral Actual Consumption Legend */}
            <div className="flex items-center space-x-2 px-2.5 py-1 rounded-lg bg-[#FF4D6D]/10 border border-[#FF4D6D]/30">
              <span className="w-3.5 h-1 rounded-full bg-[#FF4D6D] shadow-[0_0_8px_#FF4D6D]" />
              <span className="text-[#FF4D6D] font-bold">Actual (With Leaks)</span>
            </div>
          </div>
        </div>
        ...
        <div className="relative w-full overflow-x-auto">
          ...
          >
            <defs>
              {/* Glow filter for Cyan Expected Baseline */}
              <filter id="glow-cyan" x="-20%" y="-20%" width="140%" height="140%">
                <feDropShadow dx="0" dy="0" stdDeviation="3.5" floodColor="#00E5FF" floodOpacity="0.8"/>
              </filter>

              {/* Glow filter for Coral Actual Consumption */}
              <filter id="glow-coral" x="-20%" y="-20%" width="140%" height="140%">
                <feDropShadow dx="0" dy="0" stdDeviation="4" floodColor="#FF4D6D" floodOpacity="0.85"/>
              </filter>
            ...
            {/* Gradient Area Fills */}
            <path
              d={`${actualPath} L ${getX(displayedData.length - 1)} ${chartHeight - paddingY} L ${getX(0)} ${chartHeight - paddingY} Z`}
              fill="url(#actualAreaGrad)"
            />

            {/* Expected Baseline Line (Luminous Electric Cyan with Bright Dashed Glow) */}
            <path
              d={expectedPath}
              fill="none"
              stroke="#00E5FF"
            ...
            {/* Actual Consumption Line (Vibrant Coral Red) */}
            <path
              d={actualPath}
              fill="none"
              stroke="#FF4D6D"
            ...
            {displayedData.map((d, idx) => {
              ...
              return (
                <g key={d.date} className="cursor-pointer" onMouseEnter={() => setHoveredPoint(d)}>
                  ...
                  {/* Expected Baseline Point Indicator (Bright Cyan) */}
                  <circle 
                    cx={x} 
                    cy={yExpected} 
                    r={isHovered ? 4.5 : 3} 
                  ...
                  {/* Actual Consumption Point Indicator */}
                  <circle 
                    cx={x} 
                    cy={yActual} 
                    r={isHovered ? 4.5 : 3} 
                  ...
                  {/* Incident marker circle with glowing radar ripple */}
                  {d.hadIncident && (
                    <g>
                      <circle 
                        cx={x} cy={yActual} r="8" 
                        fill="#FF4D6D" fillOpacity="0.25"
                        className="animate-ping"
                      />
                      <circle 
                        cx={x} cy={yActual} r="5.5" 
                        fill="#FF4D6D" stroke="#FFFFFF" strokeWidth="1.5" 
          ...
          {hoveredPoint && (
            <div className="absolute top-2 right-4 glass-panel p-3.5 rounded-xl border border-[#00E5FF]/50 bg-[#061421]/95 text-xs font-mono space-y-1.5 shadow-2xl pointer-events-none animate-fade-in-up backdrop-blur-md">
              <div className="font-bold text-white flex items-center justify-between gap-4 border-b border-white/10 pb-1">
                <span>{hoveredPoint.date} (Day {hoveredPoint.day})</span>
                {hoveredPoint.hadIncident ? (
                  <span className="text-[10px] px-2 py-0.5 rounded-full bg-[#FF4D6D]/20 text-[#FF4D6D] border border-[#FF4D6D]/40 font-bold">
                    LEAK DETECTED
                  </span>
                ) : (
                  <span className="text-[10px] px-2 py-0.5 rounded-full bg-[#10b981]/20 text-[#10b981] border border-[#10b981]/40 font-bold">
                    STABLE
                  </span>
                )}
              </div>
              <div className="flex items-center justify-between gap-4">
                <span className="text-gray-300">Expected Baseline:</span>
                <span className="text-[#00E5FF] font-bold">{hoveredPoint.expectedLiters.toLocaleString()} L</span>
              </div>
              <div className="flex items-center justify-between gap-4">
                <span className="text-gray-300">Actual SCADA:</span>
                <span className="text-[#FF4D6D] font-bold">{hoveredPoint.actualLiters.toLocaleString()} L</span>
              </div>
              <div className="pt-1 border-t border-white/10 flex items-center justify-between gap-4 text-[#FF4D6D] font-semibold">
                <span>Unmetered Loss:</span>
                <span>+{hoveredPoint.leakLiters.toLocaleString()} L (₹{hoveredPoint.financialImpactInr.toLocaleString()})</span>
              </div>
            </div>
          )}
        </div>
