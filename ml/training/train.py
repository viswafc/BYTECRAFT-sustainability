...
Usage:
    python -m ml.training.train                       # train everything, write artifacts + reports
    python -m ml.training.train --configs B_sensor_only --models random_forest
"""
...
from ml.evaluation.metrics import evaluate_pipeline
...
def write_reports(metas: list[ModelMetadata], registry: ModelRegistry, reports_dir: Path) -> None:
    ...
    lines = [
        ...
        "legacy hyper-parameters. **All numbers below are measured by `python -m ml.training.train`; none are typed in by hand.**",
        "",
        "## Results",
        "",
